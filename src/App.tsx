...
function App() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < visualizations.length; ++i) {
      ...
      for (let j = 0; j < inputs.length; ++j) {
        ...
        if (!dataset) {
          setErrorMsgs(existingMsgs => [...existingMsgs, `Unable to access ${dataPathStr} because dataset ${dataPath[0]} is not available!`]);
          continue;
        }
        data[j] = dataset.parsed.data.map(row => +(row as {[n: string]: number})[dataPath[1]]);
        series[j] = {width: width ? width/devicePixelRatio : undefined, ...otherInputs};
      };
      const opts = {
        title,
        cursor,
      ...
      if (!plotContainer) {
        setErrorMsgs(existingMsgs => [...existingMsgs, `Plot container not found for plot-${i}`]);
        return;
      }
  ...
  function handleApplyRawConfig() {
    ...
    try {
      ...
      setErrorMsgs(existingMsgs => [...existingMsgs, "Error applying config. Please see console for more info"]);
  ...
  return (
    <div className="App">
      <div className="Content">
        ...
        <p>This <code>textarea </code> contains the raw configuration that generates the graphs above. You can copy the text below and share with others. You can also apply new configurations by replacing the text below and clicking "Apply configuration".</p>
        <textarea id="raw-config" ref={rawConfig}>{JSON.stringify({visualizations}, null, 2)}</textarea>
        <br/>
